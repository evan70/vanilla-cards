...
// Get all cards
cardsRouter.get('/', async (c: Context) => {
  const allCards = await db.select().from(cards);
  return c.json(allCards);
});

// Get card by ID
cardsRouter.get('/:id', async (c: Context) => {
...
// Create card
cardsRouter.post('/', async (c: Context) => {
...
// Update card
cardsRouter.put('/:id', async (c: Context) => {
  const idStr = c.req.param('id');
  if (!idStr) {
    return c.json({ error: 'Missing ID' }, 400);
  }
...
// Delete card
cardsRouter.delete('/:id', async (c: Context) => {
  const idStr = c.req.param('id');
  if (!idStr) {
    return c.json({ error: 'Missing ID' }, 400);
  }
